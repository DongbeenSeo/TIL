...
---
## Requirements Layer
![](https://github.com/ulgoon/fds-se/raw/develop/img/r-layer.png)
...
cycle당 목표를 설정하고 목표를 달성하면 다음 단계로 나아가는 model
...
- agile에서는 잘 사용되지 않는다.
- waterfall model의 단계를 세분화한 것 같은 과정
...
//Test code
function divideErrorTest(){
  // handling ArithmaticException 
  if(calculator.divide(4, 0) === true){
    return 'passed'
  }
  else{
    return 'failed'
